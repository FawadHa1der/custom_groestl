		...
		unsigned long long databytelen)
{
  ...
  {
    for (i = 0; i < SIZE512; i++) {
      message_block[(i%8)*8+i/8] = data[i]; //zeilenweise speichern, save line by line
    }
...
void printHexArray(uchar *array, uint size) {
    uint i;
    for(i=0; i<size; i++)
	printf("%02x", array[i]);
    printf("\n");
...
int main(int argc, char **argv) {
    ...
    FILE *file = fopen("groestl256.blb", "rb");

    // // Determine the maximum size of shared memory
    // cudaDeviceProp prop;
    // cudaGetDeviceProperties(&prop, 0);
    // maxSharedMemory = prop.sharedMemPerBlock;

    // Assuming the data file size is known and fits into memory
    // For demonstration, we assume dataSize is set correctly
    // In practice, you should obtain dataSize from the file
